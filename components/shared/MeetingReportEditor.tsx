...
const MeetingReportEditor: React.FC<Props> = ({ commission, existingReport, onClose }) => {
  ...
  const handleAddDecision = () => {
    if (newDecision.trim()) {
      const decisionObj: MeetingDecision = {
        id: Date.now().toString(),
        description: newDecision,
        votes: { for: 0, against: 0, abstain: 0 },
        status: 'pending'
      };
      setDecisions([...decisions, decisionObj]);
      setNewDecision('');
    }
  };
             ...
             <div className="space-y-8 animate-in fade-in">
                ...
                <div className="space-y-3">
                   ...
                   <div className="flex gap-2">
                      ...
                      />
                      <button 
                        onClick={() => { if(newDecision.trim()) { handleAddDecision(); }}}
                        className="px-4 bg-emerald-600 text-white rounded-xl font-bold text-xs uppercase hover:bg-emerald-700 transition-all"
                      >Ajouter</button>
                   </div>
                   <div className="space-y-2 mt-4">
                      {decisions.map((d, i) => (
                        <div key={d.id || i} className="flex items-start gap-3 p-3 bg-emerald-50/50 rounded-xl border border-emerald-100">
                           <CheckSquare size={16} className="text-emerald-600 mt-0.5 shrink-0"/>
                           <p className="text-sm font-medium text-emerald-900 flex-1">{d.description}</p>
