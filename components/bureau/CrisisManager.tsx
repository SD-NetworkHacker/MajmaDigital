...
import { Siren, AlertTriangle, Play, ShieldAlert, History, FileText, ChevronRight, Lock } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="flex items-center justify-between pb-6 border-b border-slate-800">
         <div>
             <h3 className="text-3xl font-black text-slate-100 uppercase tracking-widest flex items-center gap-3">
                <ShieldAlert size={32} className="text-red-500" /> Centre de Crise
             </h3>
             <p className="text-xs text-slate-500 mt-1">Protocoles d'intervention d'urgence pré-configurés.</p>
         </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {CRISIS_SCENARIOS.map((scenario) => (
              ...
              >
                 <div className="flex justify-between items-start mb-4">
                    <span className="px-3 py-1 bg-slate-950 rounded-lg text-[9px] font-black text-slate-400 uppercase tracking-widest border border-slate-800">{scenario.type}</span>
                 ...
                 <div className="flex items-center justify-between pt-4 border-t border-slate-800/50">
                    <span className="text-[10px] text-slate-400 font-bold uppercase flex items-center gap-2"><Lock size={12}/> {scenario.steps.length} Étapes Clés</span>
         ...
         <div className="lg:col-span-1 space-y-6">
            <div className="p-8 bg-slate-900 border border-slate-800 rounded-[2rem] flex flex-col items-center text-center h-64 justify-center relative overflow-hidden shadow-inner">
               ...
               {selectedScenario ? (
                 <div className="relative z-10 w-full animate-in zoom-in duration-300">
                    ...
                    <button 
                      ...
                      className="w-full py-4 bg-red-600 hover:bg-red-500 text-white rounded-xl font-black uppercase text-xs tracking-[0.2em] shadow-[0_0_20px_rgba(220,38,38,0.4)] transition-all active:scale-95 flex items-center justify-center gap-3"
                    >
