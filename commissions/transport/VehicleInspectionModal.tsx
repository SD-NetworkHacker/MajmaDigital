...
import { addItem, updateItem, STORAGE_KEYS } from '../../services/storage';
...
const VehicleInspectionModal: React.FC<Props> = ({ vehicleId, onClose }) => {
  ...
  const handleSubmit = () => {
      ...
      const inspectionData = {
          ...
          notes,
          inspector: 'Chef de Parc' // Mock user
      };

      // 1. Sauvegarder l'inspection
      addItem(STORAGE_KEYS.INSPECTIONS, inspectionData);

      // 2. Mettre à jour le véhicule si problème (si vehicleId est fourni et valide)
      if (vehicleId && hasIssues) {
          updateItem<Vehicle>(STORAGE_KEYS.FLEET, vehicleId, { 
              status: 'maintenance',
              maintenance: {
                  lastDate: new Date().toISOString().split('T')[0],
                  nextDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Recheck in 1 week
                  status: 'critical'
              }
          });
