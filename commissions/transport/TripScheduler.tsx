...
// Mock Passagers pour la démo (on gardera ça statique pour l'instant)
const MOCK_PASSENGERS = [
  { id: '1', name: 'Amadou Sow', phone: '77 654 32 10', seat: '1A', status: 'checked-in', type: 'Membre', amount: 3500, payment: 'Wave' },
  { id: '2', name: 'Fatou Ndiaye', phone: '76 555 44 33', seat: '1B', status: 'pending', type: 'Membre', amount: 3500, payment: 'OM' },
];
...
const TripScheduler: React.FC = () => {
  ...
  // Finance State (Mock expenses for the trip)
  const [expenses, setExpenses] = useState([
     { id: 1, label: 'Carburant (Plein Départ)', amount: 65000, type: 'fuel' },
  ]);
  ...
  const handleOpenDetail = (trip: TransportSchedule) => {
    ...
    setViewMode('detail');
    setPassengers(MOCK_PASSENGERS); // In a real app, fetch passengers for this trip
  };
  ...
  const boardingProgress = Math.round((checkedInCount / passengers.length) * 100);
  
  const financialStats = useMemo(() => {
      const revenue = passengers.reduce((acc, p) => acc + p.amount, 0);
                  ...
                  {activeTab === 'manifest' && (
                     <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                           {passengers.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(p => (
